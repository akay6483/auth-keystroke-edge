...
// Virtual Terminal Printf Override
#ifdef __GNUC__
...
int main(void)
{
  ...
  char master_password[] = "1234"; // Set your target PIN here

  // Ensure rows start LOW to enable EXTI triggers
  HAL_GPIO_WritePin(GPIOB, GPIO_PIN_0|GPIO_PIN_1|GPIO_PIN_2|GPIO_PIN_3, GPIO_PIN_RESET);

  printf("System Initialized. Enter PIN:\r\n");
  /* USER CODE END 2 */
  ...
  while (1)
  {
      ...
      {
          HAL_Delay(50); // Software debounce

          char key = scan_keypad();

          if (key != '\0')
          {
              printf("Key: %c\r\n", key); // Send to Virtual Terminal
              ...
              // Check password when 4 keys are entered
              if (buffer_index == 4)
              {
                  input_buffer[4] = '\0'; // Null-terminate

                  if (strcmp(input_buffer, master_password) == 0) {
                      printf("ACCESS GRANTED\r\n");
                      HAL_GPIO_WritePin(GPIOA, GPIO_PIN_8, GPIO_PIN_SET); // Green LED ON
                      HAL_GPIO_WritePin(GPIOA, GPIO_PIN_5, GPIO_PIN_SET); // Relay ON
                      HAL_Delay(3000); // Hold open for 3 seconds
                      HAL_GPIO_WritePin(GPIOA, GPIO_PIN_8|GPIO_PIN_5, GPIO_PIN_RESET); // Turn off
                  } else {
                      printf("ACCESS DENIED\r\n");
                      HAL_GPIO_WritePin(GPIOA, GPIO_PIN_7, GPIO_PIN_SET); // Red LED ON
                      HAL_GPIO_WritePin(GPIOA, GPIO_PIN_6, GPIO_PIN_SET); // Buzzer ON
                      HAL_Delay(1000); // Alarm for 1 second
                      HAL_GPIO_WritePin(GPIOA, GPIO_PIN_7|GPIO_PIN_6, GPIO_PIN_RESET); // Turn off
                  }
          ...
          key_pressed_flag = 0; // Clear the flag
      }
    /* USER CODE END WHILE */
...
{
    // Check if the interrupt came from Columns PB4, PB5, PB6, or PB7
...
char scan_keypad(void)
{
    // Match this layout to your Proteus Keypad
    char keys[4][4] = {
        {'7', '8', '9', '/'},
        {'4', '5', '6', 'X'},
        {'1', '2', '3', '-'},
    ...
    for (int r = 0; r < 4; r++)
    {
        ...
        // Micro-delay for hardware voltage to settle
        ...
        for (int c = 0; c < 4; c++)
        {
            ...
            {
                // Wait for the user to lift their finger (debounce)
                while(HAL_GPIO_ReadPin(GPIOB, col_pins[c]) == GPIO_PIN_RESET);

                // Reset all rows LOW so the next EXTI interrupt works
                HAL_GPIO_WritePin(GPIOB, GPIO_PIN_0|GPIO_PIN_1|GPIO_PIN_2|GPIO_PIN_3, GPIO_PIN_RESET);
                return keys[r][c];
            }
        }
    ...
    // Reset all rows LOW before exiting
    HAL_GPIO_WritePin(GPIOB, GPIO_PIN_0|GPIO_PIN_1|GPIO_PIN_2|GPIO_PIN_3, GPIO_PIN_RESET);
    return '\0';
}
/* USER CODE END 4 */
